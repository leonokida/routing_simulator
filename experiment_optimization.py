...
from routing_sim.simulation_engine.optimization_metrics import OptimizationMetrics
...
def run_experiment(topology_name: str, filename: str, sample_size: int = 30, p: float = 0.1):
    ...
    for _ in range(sample_size):
        ...
        for lambda_val in lambda_values:
            algorithm = MaxFlowRouting(lambda_val=lambda_val)
            network = Network.from_networkx_graph(topology)
            engine = SimulationEngine(network=network, allow_backtracking=True)

            # Routes without failure
            success, packet = engine.simulate_routing(
                origin, destination, algorithm, OptimizationMetrics(False),
